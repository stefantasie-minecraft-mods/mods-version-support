...
public final class SupportEvaluator {
	...
	public static ModSupport evaluate(ModKey mod, List<ModrinthVersion> versions) {
		...
		if (forFabric.isPresent()) {
			return supported(mod, forFabric.get(), false);
		}
		return best(versions)
				.map(quiltOnly -> supported(mod, quiltOnly, true))
				.orElseGet(() -> ModSupport.unsupported(mod));
	}
	...
	private static ModSupport supported(ModKey mod, ModrinthVersion version, boolean quiltOnly) {
		return ModSupport.supported(mod, version.versionNumber(), version.channel(), quiltOnly);
	}
}
